...
class Resource(object):
    ...
    def _action_recorder(self, action, expected_exceptions=tuple()):
        ...
        def lock_acquire():
            if self.stack.convergence:
                return self.LOCK_ACQUIRE
            return self.LOCK_NONE

        def lock_release():
            if self.stack.convergence:
                return self.LOCK_RELEASE
            return self.LOCK_NONE
        ...
        try:
            ...
            with excutils.save_and_reraise_exception():
                LOG.info(_LI('Update in progress for %s'), self.name)
    ...
    def update_convergence(self, template_id, resource_data, engine_id,
        ...
        def update_templ_id_and_requires(persist=True):
            ...
            if not resource_objects.Resource.select_and_update_by_id(
                ...
                LOG.info(_LI("Resource %s is locked, can't set template"),
                         six.text_type(self))
                LOG.debug(_('Resource id:%(resource_id)s locked.'
        ...
        if self.id is not None:
            if (lock == self.LOCK_NONE or self._calling_engine_id is None):
                ...
                if (lock != self.LOCK_NONE and
                    ...
                    LOG.warning(_LW('no calling_engine_id in store %s'),
                                str(rs))
            else:
    ...
    def _store_with_lock(self, rs, lock):
        ...
        if resource_objects.Resource.select_and_update_by_id(
            ...
            LOG.info(_LI('Resource %s is locked; deferring'),
                     six.text_type(self))
    ...
    def _acquire(self):
        try:
            ...
            with excutils.save_and_reraise_exception():
                LOG.info(_LI('Resource %s locked for update; _acquire failed'),
                         six.text_type(self))
        except Exception as ex:
            LOG.error(_LE('Error %s'), ex)
            raise
    ...
    def _release(self):
        try:
            ...
            LOG.warning(_LW('Failed to unlock resource %s'),
                        six.text_type(self))
        except Exception as ex:
            LOG.error(_LE('Error %s'), ex)
            raise
